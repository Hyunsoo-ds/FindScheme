...
def result():
    ...
    file_name = f'Result_{APKNAME}.txt'
...
def parse_info(res,APKNAME):
    ...
    PARAM = res['param']
    package = APKNAME[:-4]
    deeplink = f"{SCHEME}?{PARAM}=http://{SERVER_ADDR}:{str(PORT)}/t3"
    return package,deeplink
...
def analyze_apk(APK_NAME,sess):
    ...
    if(redirect_found):
        ...
        for i in redirect_found:
            ...
            package_name, deeplink = parse_info(res,APK_NAME)
            JSItest["deeplink"]=deeplink
            ...
            while True:
                ...
                if JSItest["connect"]:
                    ...
                    with open(f'JSItest_fail.txt', "w", encoding="utf-8") as file:
                        file.write(APK_NAME+'\n')
                    break
                time.sleep(1)
